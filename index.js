let student_array = [{"id":1,"first_name":"Chadwick","last_name":"Ayre","email":"cayre0@cam.ac.uk","gender":"Male","img_src":"https://robohash.org/corporisquiaperiam.png?size=50x50&set=set1","class":11,"marks":18,"passing":false,"city":"Moorreesburg"},
...
document.getElementById('table_tbody').innerHTML = `<tr>
<th id="th_id">ID</th>
<th id="th_name">Name</th> 
<th class="th_rest">Gender</th>
<th class="th_rest">Class</th>
<th class="th_rest">Marks</th>
<th class="th_rest">Passing</th>
<th class="th_rest">Email</th>
</tr>`;

student_array.forEach((item,index) => {
    let pass_status ="Failed";
...
document.getElementById('table_tbody').innerHTML += `<tr>
<td>${item.id}</td>
<td><img src=${item.img_src} height="30px"style="border:1px solid black;border-radius:50%"><span style="position:relative; bottom: 10px;">${item.first_name} ${item.last_name}</span></td>
<td>${item.gender}</td>
<td>${item.class}</td>
<td>${item.marks}</td>
<td>${pass_status}</td>
...
function SortByAtoZ() {
    student_array.sort((a, b) => a.first_name.localeCompare(b.first_name));
   

    document.getElementById('table_tbody2').innerHTML = "";
    document.getElementById('table_tbody3').innerHTML = "";
...
function SortByZtoA() {
    student_array.sort((a, b) => b.first_name.localeCompare(a.first_name));
...
function SortByMarks() {
    student_array.sort((a, b) => a.marks - b.marks);
...
function SortByPassing() {
    ...
    document.getElementById('table_tbody2').innerHTML = `<tr>
        <th id="th_id">ID</th>
        <th id="th_name">Name</th> 
        <th class="th_rest">Gender</th>
        <th class="th_rest">Class</th>
...
function SortByClass() {
    student_array.sort((a, b) => a.class - b.class);
...
function SortByGender() {
    ...
    document.getElementById('table_tbody3').innerHTML = `<tr>
        <th id="th_id">ID</th>
        <th id="th_name">Name</th> 
        <th class="th_rest">Gender</th>
        <th class="th_rest">Class</th>
...
function Search_Student() {
    ...
    let entered_value = (document.getElementById("search_input").value).toLowerCase();
    student_array.forEach((item) =>{
        let pass_status ="Failed";
        if(item.passing == true)
            {
        ...
        let full_name = (item.first_name + " " + item.last_name).toLowerCase();
        if(entered_value == (item.first_name).toLowerCase() || entered_value == (item.last_name).toLowerCase() || entered_value == full_name || entered_value == (item.email).toLowerCase())
            {
                console.log(item);
                document.getElementById('table_tbody2').innerHTML = "";
                document.getElementById('table_tbody3').innerHTML = "";
...
document.getElementById("search_form").addEventListener('click',Search_Student);